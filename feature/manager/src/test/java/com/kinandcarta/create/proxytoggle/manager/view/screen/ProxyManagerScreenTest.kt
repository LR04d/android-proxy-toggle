...
class ProxyManagerScreenTest {
    ...
    fun setUp() {
        ...
        every { mockViewModel.selectedProfile } returns MutableStateFlow(null)
        every { mockViewModel.showProfileSelector } returns MutableStateFlow(false)
        every { mockViewModel.currentProxyMode } returns MutableStateFlow(ProxyMode.GLOBAL_HTTP_PROXY)
    }
    ...
    fun `address text - WHEN changed, THEN viewModel's AddressChanged called w new text`() {
        ...
        verify {
            mockViewModel.onUserInteraction(
                match { it is ProxyManagerViewModel.UserInteraction.AddressChanged }
            )
        }
    }
    ...
    fun `port text - WHEN changed, THEN viewModel's PortChanged called w new text`() {
        ...
        verify {
            mockViewModel.onUserInteraction(
                match { it is ProxyManagerViewModel.UserInteraction.PortChanged }
            )
        }
    }
