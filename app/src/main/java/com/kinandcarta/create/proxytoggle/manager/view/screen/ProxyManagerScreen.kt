...
import com.kinandcarta.create.proxytoggle.manager.view.composable.ProxyModeSelector
...
fun ProxyManagerScreen(
    ...
    val selectedProfile by viewModel.selectedProfile.collectAsState(initial = null)
    val showProfileSelector by viewModel.showProfileSelector.collectAsState()
    val currentProxyMode by viewModel.currentProxyMode.collectAsState()
    ...
    // Collect VPN permission requests from ViewModel
    LaunchedEffect(Unit) {
    ...
    ProxyManagerScreenContent(
        ...
        currentProxyMode = currentProxyMode,
        onNavigateToProfiles = onNavigateToProfiles,
        isVpnSupported = viewModel.isVpnSupported
    )
}
...
fun ProxyManagerScreenContent(
    ...
    currentProxyMode: ProxyMode = ProxyMode.GLOBAL_HTTP_PROXY,
    onNavigateToProfiles: () -> Unit = {},
    isVpnSupported: Boolean = true
) {
        ...
        Box(
                    ...
                    if (uiState is UiState.Disconnected) {
                        ...
                        if (isVpnSupported) {
                            ProxyModeSelector(
                                ...
                                onModeSelected = {
                                    onUserInteraction(UserInteraction.ProxyModeChanged(it))
                                }
                            )
                            Spacer(Modifier.height(dimensionResource(R.dimen.default_margin)))
                        }
                        ProfilesButton(
                            ...
                            onShowProfiles = {
                                onUserInteraction(UserInteraction.ShowProfileSelector)
                            }
                        )
                    }
                }
            ...
            if (showProfileSelector) {
                ProfileSelectorBottomSheet(
                    ...
                    onProfileSelected = {
                        onUserInteraction(UserInteraction.ConnectWithProfile(it))
                    },
                    onManageProfiles = {
                        onUserInteraction(UserInteraction.HideProfileSelector)
...
private fun ProfilesButton(
    ...
    androidx.compose.material3.OutlinedButton(
        ...
        Text(
            text = if (profileCount > 0) {
                stringResource(R.string.select_profile) + " ($profileCount)"
            } else {
