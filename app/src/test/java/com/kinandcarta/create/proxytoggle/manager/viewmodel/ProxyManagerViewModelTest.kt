...
class ProxyManagerViewModelTest {
    ...
    @RelaxedMockK
    private lateinit var mockProxyModeRepository: ProxyModeRepository
    ...
    fun setUp() {
        ...
        every { mockProxyModeRepository.proxyMode } returns flowOf(ProxyMode.GLOBAL_HTTP_PROXY)
        excludeRecords { mockProxyModeRepository.proxyMode }

        every { mockProfileRepository.profiles } returns flowOf(emptyList())
        excludeRecords { mockProfileRepository.profiles }

        every { mockProfileRepository.selectedProfile } returns flowOf(null)
        excludeRecords { mockProfileRepository.selectedProfile }

        // Emit appropriate values on fake proxy-state-flow when enabling/disabling proxy
        val proxy = slot<Proxy>()
        coEvery { mockDeviceSettingsManager.enableProxy(capture(proxy), any()) } answers {
